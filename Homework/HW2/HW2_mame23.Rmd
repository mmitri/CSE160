...
# your code here
firstName <- factor(c("Clutch","Madeline","Abby","Ben","Lafayette"))
lastName <- factor(c("Mountainhawk","Doe","Johnson","Baker","Leopard"))
age <- c(35,26,22,30,23)
jobLevel <- factor(c("boss","fulltime","parttime","fulltime","parttime"), ordered = TRUE, levels = c("parttime", "fulltime","boss"))
jobTitle <- factor(c("CEO", "Programmer","Secretary","Business MGR","Inter"))
startDate <- as.Date(c("2017-05-19","2017-05-27","2018-01-02","2017-11-13","2019-05-29"))
icecreamShop <- data.frame(firstName, lastName, age, jobLevel, jobTitle, startDate)
icecreamShop
str(icecreamShop)
...
# your code here
icecreamShop$Younger <- max(icecreamShop$age) - icecreamShop$age
str(icecreamShop)
...
# your code here
print(sum(icecreamShop$jobLevel=="fulltime"))
...
# your code here
meanMalic <- mean(sdata$Malic.Acid)
for(x in sdata$Malic.Acid){
     if(x > meanMalic){
         print(x)
     }
}
