...
Instructions: Submit an R notebook that does what each of the following 13 questions requires.
...
# We provide two ways to compute, both will be given full credits
averageHeartWeight <- median(cats$Bwt[cats$Sex == "M"])
averageHeartWeight

averageHeartWeight <- with(cats, median(Bwt[Sex=="M"]))
averageHeartWeight
...
```{r}
myClasses <- c("Rauch 161", "STEPS 101", "Packard 416", "Whitaker 303") 
names(myClasses) <- c("CSE241", "MATH205", "CSE160", "PHYS11")
...
```{r}
names(myClasses[names(myClasses) != "CSE160"])

# or if you call this way, it can be acceptable

myClasses[names(myClasses) != "CSE160"]
```
...
# concatenation with comma as the seperator is counted as 0.5 pt
cat(min(vector), median(vector), max(vector), sep=', ')
```
