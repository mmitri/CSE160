...
- Try [Google's Dataset Search Service](https://datasetsearch.research.google.com/)
...
saved_cfg <- data.world::save_config("eyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiJwcm9kLXVzZXItY2xpZW50Om1hbWUyMyIsImlzcyI6ImFnZW50Om1hbWUyMzo6YWFlZWJlNzAtMWU5OS00OTA5LWE3YjItNGEyZDQ0ZTQ0YWE3IiwiaWF0IjoxNjYyOTIyMjI1LCJyb2xlIjpbInVzZXJfYXBpX3JlYWQiLCJ1c2VyX2FwaV93cml0ZSJdLCJnZW5lcmFsLXB1cnBvc2UiOnRydWUsInNhbWwiOnt9fQ.onDH8U0691c0Z_XlehS_OS6aTiwAdqs36L2UKLlKal0dPTh3ixiazxbLzmDdKiNuh5xe3VKJUGiErrXsDKkRzg")    
...
columns_wholesale <- data.world::query(
  data.world::qry_sql("SELECT * FROM TableColumns"),
  dataset = companydataset
)
columns_wholesale
tables_wholesale <- data.world::query(data.world::qry_sql("SELECT * FROM Tables"), dataset = companydataset)
tables_wholesale
```
...
stats <- data.world::query(team_query, dataset = companydataset)
dim(stats)
head(stats)
```
...
```{r}
salesRegion <- aggregate(. ~ region, stats, sum)
salesRegion$channel <- NULL
salesRegion$totSales <- rowSums(salesRegion[2:7])
salesRegion
```
...
```{r}
library(ggplot2)
salesChart <- qplot(region, totSales, data=salesRegion, geom="col")
salesChart
```
