...
g + geom_point()
```
Not all that informative.  But we can color the points by Species (as a third aesthetic).  To do so, specify an aes parameter with **color=Species** (e.g., **aes(color=Species)**).  You can also increase the size of the dots, e.g., **size = 3** as a non-aes parameter in the following code block:
```{r}
...
```{r}
g + geom_point(aes(color=Species, shape=Species), size = 3) + coord_trans(y="log10")
```

Use **geom_text()** to plot text, specifying **label=Species** as the aesthetic in addition to color:
```{r}
g + geom_text(aes(color=Species, label=Species))
```
...
```{r}
CIACountries$rfacet <- cut(CIACountries$roadways, 6)
...
```{r}
CIACountries$rfacet <- cut(log(CIACountries$roadways), 6)
table(CIACountries$rfacet)
```
...
```{r}
countries <- ggplot(data = CIACountries, aes(y = gdp, x = educ))
countries + geom_point(aes(color=net_users)) + facet_wrap(~rfacet, nrow = 1) + coord_trans(y = "log10") + xlab("Education Level") + ylab("Gross Domestic Product")
```
...
```{r}
cor(CIACountries$roadways, CIACountries$gdp, use="pairwise.complete.obs")
```
